...
Server config v6의 Shelly/Colt/Lily cooldown은 `360/390/330` tick입니다. Typed kind는 `reload_dash`/`burst_projectile`/`teleport_projectile`이고 SL-120은 검증된 config를 dispatch 경계까지 전달하되 실제 skill effect와 bot skill use는 실행하지 않습니다. Client config v3와 REST OpenAPI는 유지하고 AsyncAPI info는 `0.8.0`입니다.
