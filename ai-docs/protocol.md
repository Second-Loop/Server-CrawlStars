...
모든 matched client가 WebSocket에 연결하면 `Type: "Ready"` event를 받습니다. Ready event에는 JSON number array 형태의 `Map.map`과 `Players[].SpawnPosition`이 들어갑니다. Ready event의 team/slot/spawn과 실제 `simulation.State` 초기 값은 같은 room-local assignment 결과를 사용합니다. 모든 client가 `{"Type":"ready"}`를 보내면 server는 `Snapshot.status: "starting"`과 `Snapshot.countdown: 5`를 1번 broadcast합니다. Client는 이 신호를 기준으로 fake timer를 표시하고, server는 5초를 내부에서 센 뒤 `Snapshot.status: "started"`를 보낸 다음 simulation ticker를 시작합니다.
