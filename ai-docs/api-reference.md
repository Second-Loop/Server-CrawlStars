...
Request body와 `characterType`은 required입니다. `gameMode`만 생략하거나 빈 문자열로 보내면 기본 `duel_1v1`을 선택합니다.

Client는 `duel_1v1`, `solo`, `team` 중 하나와 stable `characterType` `0=Shelly`, `1=Colt`, `2=Lily`를 보냅니다. characterType 누락, explicit `null`, non-integer, string/bool/object/array, 지원하지 않는 integer는 400 `invalid_character_type`이며 room/player/session을 만들지 않습니다.
...
- Loading 진입 시 별도의 strict 30초 room-owned Ready ACK deadline을 시작합니다. 정확한 경계(`now >= deadline`)의 ACK도 거부하며 timer worker가 늦게 실행돼도 pre-start room 전체, session credential, player ID를 취소합니다.
...
- 모든 human Ready ACK가 경계 전에 모이면 Ready deadline을 해제하고 Starting으로 한 번만 전이합니다. Bot ACK는 필요하지 않습니다. Participant replacement는 제공하지 않습니다. Started match의 비의도적 `peer_close`, read/write failure, Ping failure/timeout, control overflow는 10초 reconnect grace를 사용하고, grace 중 simulation은 계속됩니다. Unmatched disconnect는 room-owned 10초 fill deadline과 credential을 유지하고, matched/loading/starting disconnect는 pre-start cancel로 room을 삭제합니다.
...
기본 map source는 server binary가 embed한 `server-config/game-config.json`의 `map`입니다. 현재 기본 map은 Client PR #28에서 merge된 `Map_0`과 값이 같은 40x40 grid이며 `index=0`, `maxPlayers=6`, spawn tile 2개가 아니라 정확히 6개입니다. Client `main` `50f10c27a575c2bc8f53c7e7b3385de69876184c`, last-changing commit `4f3292603e6809e918f609e5be8dd03d3ded8988`, blob `89228cead52df257a0489101d045b3d288634e27`, canonical semantic SHA-256 `b1729488ec19efb433d19df112b88f1fd1b33a1f39f15fb1cb4df0f93d9f8e60`을 exact-grid Go regression에 고정합니다. 이 문서의 예시는 간결함을 위해 명시적 test/dev용 5x5 `StaticGameConfig()`를 사용합니다. Production은 config 로드나 검증에 실패하면 listener를 열기 전에 startup을 실패시키며 5x5 map으로 fallback하지 않습니다. `internal/simulation/fixtures/default-map.json`은 테스트용 fixture로만 남아 있습니다.
...
Input `PressedAttack`은 server config v7의 캐릭터별 일반 공격 activation 요청입니다. Shelly는 같은 tick에 5발, Colt는 activation tick `A` 기준 `A+[0,3,6,9,12,15]`에 6발, Lily는 2.2 tile centerline melee를 실행합니다. Colt의 후속 emission은 새 activation이 아니므로 snapshot `PressedAttack`은 activation tick에만 `true`입니다.
...
Skill-ready와 non-zero direction이면 normal attack보다 먼저 승인합니다. Shelly `reload_dash`는 일반 이동 전에 charge를 max로 만들고 recharge를 `0`으로 초기화한 다음 `2.7 tile = 3.24 world` 대시를 9 tick에 나눠 실행해요. Wall/Water/boundary/live player의 최초 swept-circle 접촉 직전에 멈추며 partial/blocked dash도 reload와 cooldown을 환불하지 않습니다. Colt `burst_projectile`은 승인 방향을 고정하고 `S+[0,2,4,6,7,9,11,13,14,16]`에 `Type: colt_skill`, damage `320`, range `9.35 tile`, speed `13`, radius `0.3`인 10발을 emission tick 현재 위치에서 생성합니다. 승인 시 normal burst 미래분을 교체하고 마지막 발 tick까지 일반 공격의 charge 소비와 queue를 막으며 owner 사망은 아직 commit되지 않은 emission을 취소합니다. Lily `teleport_projectile`은 `Type: lily_seed`, damage `400`, range `12.48 tile`, speed `13`, radius `0.3`인 씨앗 하나를 생성합니다. 적중 시 target의 피격 전 위치를 보존해 피해를 먼저 적용하고 살아 있는 Lily를 seed 방향의 target 뒤 `1 tile`로 옮깁니다. Wall/Water/boundary/다른 live player에 막히면 같은 ray의 최소 비겹침 거리 이상에서 가장 큰 유효 지점까지 backoff하며, target이 죽어도 이동하고 Lily 사망 또는 유효 지점 부재에는 피해만 유지합니다. Snapshot `Pos`, `PressedSkill`, `SkillReadyTick`, `AttackCharges`, `NextAttackChargeTick`, `Projectiles[].Type`이 승인 결과를 보여 줍니다. Cooldown 또는 zero direction이면 기존 normal attack 판정으로 fall through합니다. Ready predicate는 `Snapshot.Tick >= SkillReadyTick`이고 tick `A` 승인에 cooldown `C`를 적용하면 `A + C`를 기록해 exact `A + C` tick도 허용합니다. Server config v7 cooldown은 Shelly/Colt/Lily 순서로 `360/390/330`입니다. Bot skill use는 아직 실행하지 않습니다. AsyncAPI는 `0.10.0`이고 REST OpenAPI와 Client config v3는 변경하지 않습니다.
...
Snapshot의 `Players[].PressedSkill`은 skill이 승인된 tick에만 `true`인 transient 결과이고, `Players[].SkillReadyTick`은 다음 readiness를 결정하는 persistent canonical state입니다. `Players[].AttackCharges`는 현재 charge, `Players[].NextAttackChargeTick`은 다음 복구 absolute tick이며 max charge에서 `0`입니다. 초기 skill 값은 `false/0`이며 control snapshot의 `Players: null`, `Projectiles: null`은 그대로 유지합니다.

Snapshot `Projectiles[]`는 기존 `Damage`와 `Type` wire field를 그대로 씁니다. `Damage`는 owner 캐릭터의 공격 설정에서 복사하고, `Type`은 server config v7의 `normalAttack.projectile.type` 또는 typed `skill.projectile.type`이 가리키는 ID입니다. Canonical ID는 일반 공격용 `default`, Colt 스킬 10발용 `colt_skill`, Lily 순간이동 씨앗용 `lily_seed`입니다. Projectile은 configured range endpoint까지 clamp한 위치에서 Wall/boundary 충돌, player hit, range 만료 순서로 판정하므로 endpoint tangent hit도 포함됩니다. 같은 위치의 eligible target은 `PlayerID` 오름차순으로 하나를 선택합니다. Destroyed projectile은 destroyed snapshot tick `D`부터 `D+29`까지 정확히 30개 gameplay snapshot에 `IsDestroyed: true`로 남고 `D+30` 전에 제거됩니다. 느린 writer의 latest-only coalescing으로 개별 tombstone snapshot은 누락될 수 있어 Client absent-ID reconciliation이 필요합니다. `lily_seed`의 피해와 순간이동은 같은 적중 snapshot의 `Players[].HP/IsDead/Pos`로 나타나고 별도 effect event는 없습니다.
...
Projectile hit은 room이 시작할 때 고정한 selected mode rules를 사용합니다. 모든 mode에서 owner와 이미 사망한 player를 제외하고, Solo는 나머지 live player를 모두 적으로 봅니다. 현재 `friendlyFire=false`인 Team/Duel은 ally를 통과하고 enemy만 hit합니다. 같은 tick에 여러 eligible target이 겹치면 `PlayerID` 오름차순 첫 target만 피해를 받고 projectile이 destroy됩니다.
...
Field 이름은 Unity prototype과 맞춰 `ClientTick`, `MoveDir`, `AttackDir`, `PressedAttack`, `PressedSkill`, `Id`, `IsBot`, `OwnerId`, `Pos`, `Dir`, `HP`, `SkillReadyTick`, `IsDead`, `LastProcessedClientTick`, `IsDestroyed`처럼 유지합니다.
...
- projectile speed: 13
- projectile damage: owner의 `normalAttack.damagePerHit` (`252/340`; Lily `1210`은 melee)
...
- `server-config/game-config.json`: server binary가 embed해서 room store와 simulation 기본값으로 쓰는 server-only v7 config입니다. `tickRate`, `tile.size`, player type별 `normalAttack`과 typed `skill.kind`/`skill.cooldownTicks`, server-only bot tuning, player/projectile type별 runtime 값, `mode.default`와 `mode.catalog`, `map`을 포함합니다. Charge/recharge는 Shelly `3/30`, Colt `3/30`, Lily `2/30`이고 skill cooldown은 `360/390/330`입니다. Colt 일반/스킬 exact offset, Shelly `2.7 tile`, Lily `12.48 tile`이 canonical입니다. Gameplay snapshot은 `AttackCharges`와 `NextAttackChargeTick`을 추가로 공개합니다.
...
## SL-116 Bot 행동과 server-only config

SL-116의 `server-only config v5`는 client artifact나 REST/WebSocket payload가 아니라 server binary가 읽는 행동 tuning입니다.
...
Room은 이전 authoritative `Players`와 `Projectiles`를 한 번 관측해 모든 bot input을 만들고 PlayerID 오름차순으로 합쳐 `State.Step`을 한 번 실행합니다. 행동 priority는 dodge → explore → retreat → chase이고, 공격 판단은 이동과 독립입니다. Bot은 `PressedSkill: false`만 요청하며, only an approved snapshot with `PressedAttack: true` updates cadence입니다.

Client config v3와 public REST/OpenAPI/AsyncAPI field/event shape는 unchanged입니다. 공개 REST에는 bot endpoint가 없습니다. AsyncAPI info version `0.7.0`도 유지되므로 bot tuning은 API response나 WebSocket schema에서 찾지 않습니다.
...
## SL-98 CharacterType 필수 계약

`POST /matchmaking/join`의 required lower-camel `characterType`은 stable ID `0=Shelly`, `1=Colt`, `2=Lily`를 받습니다. 누락, explicit `null`, non-integer, string/bool/object/array, 지원하지 않는 정수는 모두 400 `invalid_character_type`이고 room/player/session을 만들지 않습니다. 누락값 Shelly fallback과 `character_type_defaulted` 경고는 제거됐습니다.

REST `Player.characterType`은 required이며 top-level `player`와 nested `room.players[]`가 같은 값을 반환합니다. WebSocket Ready와 gameplay Snapshot은 required PascalCase `CharacterType`으로 canonical participant identity를 보존합니다. Bot participant는 생성 시 선택된 canonical 값을 match 동안 유지하며, `0=Shelly`, `1=Colt`, `2=Lily` 중 균등·독립적으로 선택되고 중복될 수 있습니다. Debug human participant의 기존 기본값은 Shelly `0`입니다. Client config v3는 같은 stable numeric type과 client 표시·입력 보조값을 제공하고, server config v7의 현재 authoritative stats는 Shelly `4000`, Colt `3100`, Lily `4100` HP와 `3/3/2` attack charge, 공통 30 tick recharge, skill cooldown `360/390/330`입니다.
