...
Skill-ready와 non-zero direction이면 normal attack보다 먼저 승인하고 attack charge를 보존합니다. Cooldown 또는 zero direction이면 기존 normal attack 판정으로 fall through합니다. Snapshot `PlayerData.PressedSkill`은 해당 tick의 transient server approval pulse이고, `SkillReadyTick`은 persistent canonical absolute ready tick입니다. `AttackCharges`는 현재 일반 공격 charge, `NextAttackChargeTick`은 다음 복구 absolute tick이며 max charge이면 `0`입니다. Ready predicate는 `Snapshot.Tick >= SkillReadyTick`이고 tick `A` 승인에 cooldown `C`를 적용하면 `A + C`를 기록해 exact `A + C` tick도 허용합니다. Server config v6 cooldown은 Shelly/Colt/Lily 순서로 `360/390/330` tick이고 typed skill config를 dispatch 경계까지 전달하지만 SL-120은 실제 effect를 실행하지 않습니다. AsyncAPI는 `0.8.0`이고 REST OpenAPI와 Client config v3는 변경하지 않습니다.
