...
Input `PressedAttack`은 server config v6의 캐릭터별 일반 공격 activation 요청입니다. Shelly는 같은 tick에 5발, Colt는 activation tick `A` 기준 `A+[0,3,6,9,12,15]`에 6발, Lily는 2.2 tile centerline melee를 실행합니다. Colt의 후속 emission은 새 activation이 아니므로 snapshot `PressedAttack`은 activation tick에만 `true`입니다.
...
Skill-ready와 non-zero direction이면 normal attack보다 먼저 승인합니다. Shelly `reload_dash`는 일반 이동 뒤 charge를 max로 만들고 recharge를 `0`으로 초기화한 다음 `5.4 tile = 6.48 world` 대시를 실행합니다. Wall/Water/boundary/live player의 최초 swept-circle 접촉 직전에 멈추며 partial/blocked dash도 reload와 cooldown을 환불하지 않습니다. Colt `burst_projectile`은 승인 방향을 고정하고 `S+[0,2,4,6,7,9,11,13,14,16,18,20]`에 `Type: colt_skill`, damage `320`, range `11 tile`, speed `13`, radius `0.3`인 12발을 emission tick 현재 위치에서 생성합니다. 승인 시 normal burst 미래분을 교체하고 마지막 발 tick까지 일반 공격의 charge 소비와 queue를 막으며 owner 사망은 아직 commit되지 않은 emission을 취소합니다. Lily `teleport_projectile`은 `Type: lily_seed`, damage `400`, range `10.4 tile`, speed `13`, radius `0.3`인 씨앗 하나를 생성합니다. 적중 시 target의 피격 전 위치를 보존해 피해를 먼저 적용하고 살아 있는 Lily를 seed 방향의 target 뒤 `1 tile`로 옮깁니다. Wall/Water/boundary/다른 live player에 막히면 같은 ray의 최소 비겹침 거리 이상에서 가장 큰 유효 지점까지 backoff하며, target이 죽어도 이동하고 Lily 사망 또는 유효 지점 부재에는 피해만 유지합니다. Snapshot `Pos`, `PressedSkill`, `SkillReadyTick`, `AttackCharges`, `NextAttackChargeTick`, `Projectiles[].Type`이 승인 결과를 보여 줍니다. Cooldown 또는 zero direction이면 기존 normal attack 판정으로 fall through합니다. Ready predicate는 `Snapshot.Tick >= SkillReadyTick`이고 tick `A` 승인에 cooldown `C`를 적용하면 `A + C`를 기록해 exact `A + C` tick도 허용합니다. Server config v6 cooldown은 Shelly/Colt/Lily 순서로 `360/390/330`입니다. Bot skill use는 아직 실행하지 않습니다. AsyncAPI는 `0.9.0`이고 REST OpenAPI와 Client config v3는 변경하지 않습니다.
