...
Room terminal decision에서는 ticker를 terminal decision 즉시 중단하고 tick observer, encode, enqueue를 진행합니다. Normal cleanup은 모든 captured session의 `closeDone` 뒤 registry를 분리하고 connected/active observer를 반영한 다음 player ID를 release하고 `room_ended` log와 resource close를 수행하며 cleanup success signal은 마지막에 닫습니다. Hard TTL과 debug removal은 ending room을 제거하지 않습니다.
