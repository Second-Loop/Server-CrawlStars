...
Skill-ready와 non-zero direction이면 normal attack보다 먼저 승인합니다. Shelly `reload_dash`는 일반 이동 뒤 charge를 max로 만들고 recharge를 `0`으로 초기화한 다음 `5.4 tile = 6.48 world` 대시를 실행합니다. Wall/Water/boundary/live player의 최초 swept-circle 접촉 직전에 멈추며 partial/blocked dash도 reload와 cooldown을 환불하지 않습니다. Snapshot `Pos`, `PressedSkill`, `SkillReadyTick`, `AttackCharges`, `NextAttackChargeTick`이 한 승인 결과를 함께 보여 줍니다. Cooldown 또는 zero direction이면 기존 normal attack 판정으로 fall through합니다. Ready predicate는 `Snapshot.Tick >= SkillReadyTick`이고 tick `A` 승인에 cooldown `C`를 적용하면 `A + C`를 기록해 exact `A + C` tick도 허용합니다. Server config v6 cooldown은 Shelly/Colt/Lily 순서로 `360/390/330`입니다. Colt/Lily effect와 bot skill use는 아직 실행하지 않습니다. AsyncAPI는 `0.8.0`이고 REST OpenAPI와 Client config v3는 변경하지 않습니다.
