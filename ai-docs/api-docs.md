...
- REST interactive page는 server binary에 embed되는 Swagger UI wrapper로 render합니다.
- OpenAPI `servers`의 첫 항목은 배포된 Swagger UI에서 바로 debug request를 보낼 수 있도록 현재 server origin(`/`)으로 둡니다.
...
REST는 각 operation이 bounded request와 response를 가지므로 OpenAPI가 적합합니다. Server-hosted Swagger UI는 endpoint를 읽기 쉽게 만들고 browser에서 debug request를 시도할 수 있게 합니다. 배포된 `/openapi`에서는 같은 origin의 API를 기본 server로 사용하고, local 개발에서는 `http://localhost:8080` server 선택지를 사용할 수 있습니다.
...
`/openapi`와 `/asyncapi`는 human-readable UI입니다. `/openapi`는 Swagger UI CDN asset을 사용해 raw OpenAPI spec을 render하고, `/asyncapi`는 repository-owned static HTML입니다. `/openapi.yaml`과 `/asyncapi.yaml`은 raw spec입니다. OpenAPI `servers`는 현재 server origin(`/`)을 첫 항목으로 두어 배포된 Swagger UI의 Try it out이 localhost로 고정되지 않게 합니다.

Docs UI는 `docs-ui`에서 dependency-free Node script로 build하고, build 결과는 `internal/docs/static/`과 `internal/docs/api/`에 생성한 뒤 Go `embed`로 server binary에 포함합니다. 생성물은 commit하지 않습니다. 따라서 clean checkout에서 Go compile/test/build를 실행하기 전에는 `make docs-build` 또는 `make ci`가 먼저 필요합니다.
...
처음으로 spec file을 추가하거나 계약 surface를 바꾸는 implementation issue는 해당 spec validation도 함께 확인해야 합니다. 최소 기준:

- OpenAPI spec의 version, stability marker, 기본 same-origin server, 필수 path/schema marker를 확인합니다.
- AsyncAPI spec의 version, stability marker, channel/schema marker를 확인합니다.
- Documentation path가 `ai-docs/protocol.md`에 언급되어 있습니다.
- `make ci`가 spec validation, docs build, Go validation을 함께 실행합니다.

SL-51부터 build path는 npm package install을 요구하지 않습니다. Validation은 raw spec의 필수 version/stability/path/schema marker를 확인하는 dependency-free consistency check로 유지합니다. Full schema validation이 필요해지면 별도 issue에서 CLI/tool 도입 범위를 정합니다.
