...
AsyncAPI에는 message schema뿐 아니라 연결 생명주기도 기록합니다. Server heartbeat는 30초 간격이고 Ping마다 90초 deadline을 사용합니다. 일반 gameplay snapshot만 client별 latest-only로 합치며, Ready/lifecycle/error control과 `terminal snapshot -> GameEnd -> close`는 reliable ordering을 유지합니다. Payload write는 매번 새 5초 context를 사용합니다. Ping/read/write failure는 같은 close-once 정책으로 현재 session만 해제하며 bot replacement와 reconnect grace는 범위 밖입니다.
...
`docs-ui/scripts/validate.mjs`는 raw spec의 필수 version, stability marker, security count, opaque pattern, error/status marker, tokenized path, path-only binding, public DTO secret 부재를 확인합니다. 이 marker validator는 YAML schema parser가 아니므로 OpenAPI/AsyncAPI YAML parse와 공식 AsyncAPI CLI validation을 함께 실행합니다.
