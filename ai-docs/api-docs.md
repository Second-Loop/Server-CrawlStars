...
REST는 각 operation이 bounded request와 response를 가지므로 OpenAPI가 적합합니다. Server-hosted Swagger UI는 endpoint를 읽기 쉽게 만들고 browser에서 debug request를 시도할 수 있게 합니다.
...
`/openapi`와 `/asyncapi`는 human-readable UI입니다. `/openapi`는 Swagger UI CDN asset을 사용해 raw OpenAPI spec을 render하고, `/asyncapi`는 repository-owned static HTML입니다. `/openapi.yaml`과 `/asyncapi.yaml`은 raw spec입니다.
...
- OpenAPI spec의 version, stability marker, 필수 path/schema marker를 확인합니다.
