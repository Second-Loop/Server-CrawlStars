...
`make ci`와 `make docs-build`는 source marker → pinned Redocly/AsyncAPI schema 검증 → docs build 순서를 강제해요. 배포 패키지 workflow도 `make docs-build`를 호출해 같은 gate를 거쳐요. `make docs-schema`로 build 없이 공식 검증까지 실행할 수 있어요. 첫 실행에는 Node 22의 npx와 npm registry 접근이 필요해요.
