...
REDOCLY_TELEMETRY=off REDOCLY_SUPPRESS_UPDATE_NOTICE=true npx --yes --package @redocly/cli@2.38.0 redocly lint --extends=minimal api/openapi.yaml
npx --yes --package @asyncapi/cli@6.0.2 asyncapi validate api/asyncapi.yaml --fail-severity=error
node docs-ui/scripts/build.mjs
mise exec -- env GOCACHE="$PWD/.cache/go-build" GOMODCACHE="$PWD/.cache/go-mod" go test ./internal/docs -count=1
```
...
`make ci`와 `make docs-build`는 source marker → pinned Redocly/AsyncAPI schema 검증 → docs build 순서를 강제해요. `make vuln-check`는 같은 docs build 뒤 pinned govulncheck v1.7.0으로 호출 경로의 알려진 취약점을 검사해요. CI와 배포 패키지 workflow에 이 보안 gate를 포함해요. `make docs-schema`로 build 없이 공식 schema 검증까지 실행할 수 있어요. 첫 실행에는 Node 22의 npx, npm registry, Go module proxy와 Go vulnerability DB 접근이 필요해요.
