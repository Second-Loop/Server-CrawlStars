...
2. 만료된 room을 정리합니다.
3. 여유 있는 waiting room을 찾습니다.
4. 없으면 새 waiting room을 만듭니다.
5. player와 session token을 발급합니다.
6. active mode의 required player 수가 차면 matchmaking room으로 잠그고 late join을 막습니다.
7. `room`, `player`, `sessionToken`, tokenized `webSocketPath`를 반환합니다.
