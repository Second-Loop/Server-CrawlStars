...
클라이언트는 `POST /matchmaking/join`의 optional `gameMode`로 `duel_1v1`, `solo`, `team`을 고르고, 같은 mode의 `room`, human `player`, `sessionToken`, tokenized `webSocketPath`를 받습니다. 서버는 sessionless bot도 공통 `InputCommand`로 합쳐 room-local selected config의 tick에서 `State.Step(inputs) -> Snapshot`을 정확히 한 번 실행하고, 같은 config로 Solo 마지막 생존자와 Team elimination까지 판정합니다.
...
internal/simulation
  ...
  map, input 검증과 PlayerID 정렬, movement, projectile, attack charge, mode별 hit, HP/death rule
...
한 tick 안에 같은 human player가 여러 input을 보내면 마지막 input만 사용합니다. Pending map key가 authoritative `PlayerID`라 payload에 섞인 ID를 신뢰하지 않습니다. Bot key로 들어온 외부 input은 버리고 직전 authoritative snapshot을 읽은 pure controller 결과로 대체합니다. Human/bot command를 `PlayerID` 오름차순으로 합친 뒤 같은 `State.Step`에 전달합니다. 잘못된 JSON은 `invalid_input` error message를 보내고 snapshot stream은 유지합니다.

### 6. Tick 처리

Started room의 tick 흐름:

1. `room.mu`가 보호하는 직전 authoritative `lastPlayers` snapshot을 읽습니다.
2. Pure bot controller가 nearest live enemy를 골라 공통 `InputCommand`를 만듭니다. 같은 거리는 `PlayerID`, 같은 좌표 방향은 `+X`로 결정합니다.
3. Pending map key를 authoritative human ID로 쓰고 bot key input을 controller 결과로 대체한 뒤 `PlayerID` 오름차순으로 정렬합니다.
4. Pending map을 비우고 `room.state.Step(inputs)`를 정확히 한 번 호출합니다.
5. 반환 snapshot을 다음 tick의 `lastPlayers`로 복사하고 Room REST detail/list의 `latestSnapshot` summary를 바로 갱신합니다.
...
Bot controller는 이동이나 피해를 직접 계산하지 않습니다. Movement, projectile, hit, HP/death, attack charge는 모두 `internal/simulation.State.Step`이 계속 소유합니다.
