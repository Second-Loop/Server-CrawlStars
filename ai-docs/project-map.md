...
Required player가 모두 join해도 REST `room.status`는 `waiting`입니다. Selected mode의 required player 2명 또는 6명이 각각 WebSocket에 연결하면 모든 connection이 같은 `Ready` event를 받고, 각 human player의 ready ACK가 모두 모이면 `starting/countdown: 5`를 한 번 broadcast합니다. Duplicate ACK는 player identity별 quorum을 늘리지 않습니다. 5초 뒤 `started`를 한 번 보내고 room-local gameplay ticker 하나를 시작합니다. Ready timeout, reconnect grace, participant replacement, bot fill은 없으며 start 전 실제 disconnect는 match cancel입니다.
...
1. Selected mode의 required 2명 또는 6명이 서로 다른 WebSocket session으로 모두 연결되면 모든 connection이 같은 `{"Type":"Ready","Map":...,"Players":[...]}`를 받습니다.
2. client는 이 map과 `Players[].SpawnPosition`으로 렌더 준비를 끝낸 뒤 `{"Type":"ready"}`를 보냅니다.
3. Required human player identity 2개 또는 6개가 모두 ready가 되면 `Snapshot.status: "starting"`과 `Snapshot.countdown: 5`가 connection당 1번 broadcast됩니다. Duplicate ACK는 idempotent합니다.
...
- matchmaking: room-local selected mode의 required 2개 또는 6개 WebSocket session이 각각 Ready ACK를 보내고 server 내부 countdown이 끝난 시점
...
Normal GameEnd cleanup은 terminal decision 때 captured한 모든 session의 `closeDone`을 기다립니다. 그 뒤 registry, connected/active observer, player ID, `room_ended` log, resources를 정리하고 cleanup success signal을 마지막에 닫습니다. Hard TTL과 debug clear/delete는 ending room을 제거하지 않습니다.
