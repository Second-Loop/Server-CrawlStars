...
server config v6가 일반 공격 실행의 source of truth입니다. 각 player type의 `normalAttack`이 kind, hit당 damage, tile range, `3/3/2` max charge, 30 tick recharge와 projectile schedule을 소유하고, projectile type catalog는 radius/speed를 소유합니다. Client config v3는 조준·cooldown UI와 로컬 bot 입력 보조값만 제공하며 authoritative combat stat을 대체하지 않습니다.
...
Skill-ready와 non-zero direction이면 normal attack보다 먼저 승인합니다. Server config v6의 typed `SkillConfig`가 `reload_dash`/`burst_projectile`/`teleport_projectile` payload를 소유하고 승인 경계가 이를 character executor에 전달합니다. Shelly executor는 charge/recharge를 `max/0`으로 만든 뒤 일반 이동이 끝난 위치에서 `5.4 tile` swept dash intent를 만들고, 모든 same-tick dash를 연속 시간 batch로 정산합니다. Wall/Water/boundary/live player는 막고 dead player는 통과하며 partial/blocked dash도 reload와 cooldown을 유지합니다. Colt executor는 기존 tick scheduler의 active normal burst를 skill burst로 교체하고 승인 방향을 고정해 `S+[0,2,4,6,7,9,11,13,14,16,18,20]`에 `colt_skill` 12발을 생성합니다. 각 발은 emission tick의 post-movement 위치에서 시작하고 마지막 발 tick까지 일반 공격의 charge 소비와 queue를 막습니다. Lily executor는 `lily_seed` 하나를 생성하고 projectile pre-phase 적중 시 target의 피격 전 위치를 보존해 damage `400`을 먼저 적용한 뒤 살아 있는 Lily를 seed 방향의 target 뒤 `1 tile`로 순간이동합니다. 목적지 Wall/Water/boundary/live player 충돌 구간을 합쳐 같은 ray의 가장 큰 유효 거리를 선택하고, target 사망은 이동을 취소하지 않지만 owner 사망이나 유효 지점 부재는 피해만 유지합니다. Cooldown 또는 zero direction이면 기존 normal attack 판정으로 fall through합니다. Public AsyncAPI는 `0.8.0`이며 gameplay `PlayerData.PressedSkill`/`SkillReadyTick`/`AttackCharges`/`NextAttackChargeTick`, 기존 `Pos`와 `Projectiles[].Type`으로 결과를 공개합니다. REST OpenAPI와 starting/started control의 `Players: null`, `Projectiles: null`은 유지하고 bot skill use는 후속 티켓 범위입니다.
