...
Skill-ready와 non-zero direction이면 normal attack보다 먼저 승인합니다. Server config v7의 typed `SkillConfig`가 `reload_dash`/`burst_projectile`/`teleport_projectile` payload를 소유하고 승인 경계가 이를 character executor에 전달합니다. Shelly executor는 일반 이동 전에 charge/recharge를 `max/0`으로 만들고 고정 방향, 구간 거리, 남은 tick을 보관해요. `2.7 tile`을 `dashDurationTicks: 9`로 나누고 매 tick의 모든 dash segment를 연속 시간 batch로 정산해요. Wall/Water/boundary/live player는 막고 dead player는 통과하며 partial/blocked dash도 reload와 cooldown을 유지합니다. Colt executor는 기존 tick scheduler의 active normal burst를 skill burst로 교체하고 승인 방향을 고정해 `S+[0,2,4,6,7,9,11,13,14,16]`에 `colt_skill` 10발을 생성합니다. 각 발은 emission tick의 post-movement 위치에서 시작하고 마지막 발 tick까지 일반 공격의 charge 소비와 queue를 막습니다. Lily executor는 `lily_seed` 하나를 생성하고 projectile pre-phase 적중 시 target의 피격 전 위치를 보존해 damage `400`을 먼저 적용한 뒤 살아 있는 Lily를 seed 방향의 target 뒤 `1 tile`로 순간이동합니다. 목적지 Wall/Water/boundary/live player 충돌 구간을 합쳐 같은 ray의 가장 큰 유효 거리를 선택하고, target 사망은 이동을 취소하지 않지만 owner 사망이나 유효 지점 부재는 피해만 유지합니다. Cooldown 또는 zero direction이면 기존 normal attack 판정으로 fall through합니다. Public AsyncAPI는 `0.10.0`이며 gameplay `PlayerData.PressedSkill`/`SkillReadyTick`/`AttackCharges`/`NextAttackChargeTick`, 기존 `Pos`와 `Projectiles[].Type`으로 결과를 공개합니다. REST OpenAPI와 starting/started control의 `Players: null`, `Projectiles: null`은 유지하고 bot skill use는 후속 티켓 범위입니다.
...
- 한 projectile이 여러 eligible target과 겹치면 `PlayerID` 오름차순 첫 target만 hit합니다.
...
Gameplay config는 client 공유용과 server runtime용을 분리합니다. `client-config/game-config.json`은 Client CI가 sparse checkout해 Unity runtime asset 경로로 복사하는 client config v3 artifact입니다. Stable `type 0/1/2`, Unity world unit의 `normalAttackDistance`·`skillAttackDistance`, 초 단위 `normalAttackCoolDown`·`skillAttackCoolDown`, client charge 표현용 `maxBullets`를 담습니다. Server의 Go parser가 canonical artifact를 검증하고 Client 소비 계약은 필수 field와 version을 build/runtime 양쪽에서 거부하도록 요구합니다. `server-config/game-config.json` v7는 server binary가 embed해서 room store와 simulation 기본값으로 사용하는 canonical runtime config이며 tick rate, speed `2`, radius `0.5`, HP `4000/3100/4100`, 캐릭터별 `normalAttack`, typed skill kind와 `skill.cooldownTicks`, server-only bot tuning, `mode.default`와 `mode.catalog`, map을 담습니다. 실제 hit/range/charge와 스킬 승인 결과는 server-authoritative state와 snapshot이 최종 truth이며 client 설정으로 gameplay를 재판정하지 않습니다. Skill cooldown의 public 경계는 gameplay `PlayerData.SkillReadyTick`이고 현재 탄약은 `AttackCharges`, 다음 복구는 `NextAttackChargeTick`으로 공개합니다.

## SL-82 CharacterType ownership

Client config v3의 `characters[].type`과 API 계약이 `0=Shelly`, `1=Colt`, `2=Lily` stable identity mapping을 공유합니다. Server config v7는 같은 ID에 대한 HP `4000/3100/4100`, `3/3/2` attack charge, runtime combat stat과 `skill.cooldownTicks`를 소유하고 simulation의 canonical `PlayerData.SkillReadyTick`, `AttackCharges`, `NextAttackChargeTick`으로 상태를 공개합니다. `internal/rooms`는 join 선택을 canonical participant에 저장하고 REST/Ready/Snapshot transport casing으로 변환합니다. `internal/simulation`은 이미 저장된 type의 stat을 적용합니다. 따라서 join parsing, participant identity, simulation stat 적용을 서로 다른 owner가 다시 선택하지 않습니다.
...
- Room이 `room-owned controller state`와 bot별 next-attack cadence를 `room.mu` 아래 소유합니다. State에는 `exploreEpoch`, 목적지, `(start, goal)` path cache와 first-step tile을 보관하고 bot 제거/room cleanup 때 cadence와 함께 폐기합니다.
...
- A*는 4방향(상·하·좌·우)만 사용하고 Wall과 Water를 blocked로 봅니다. `G=1`, Manhattan `H`, open-set 동률은 `F -> H -> y -> x` 오름차순입니다. Cache는 cardinal direction이 아니라 first-step tile을 저장합니다. 경로가 새 축으로 꺾일 때는 authoritative position을 현재 tile의 수직축 중앙에 먼저 맞춘 뒤 first-step 방향으로 진행해 player radius가 Wall/Water 모서리를 긁지 않게 합니다. Invalid/blocked start·goal, disconnected map, open set 소진은 path failure이며 explore는 목적지를 폐기하고 chase/retreat는 그 tick 이동을 zero로 둡니다.
- Human pending input과 모든 bot raw input을 먼저 모아 이번 tick의 상대 이동을 계산합니다. 최종 이동 후보끼리 swept collision이 예상되면 bot은 현재 진행 방향 기준 `+90°`, `-90°` 순서로 map/player-safe 후보를 고릅니다. 같은 previous snapshot을 보는 정면 충돌 bot은 서로 반대 world 방향으로 비켜나며, 후보가 모두 막히면 zero movement입니다. Simulation의 공통 player collision 판정은 변경하지 않습니다.
...
- `server-config/game-config.json` v7는 SL-116의 server-only bot tuning을 exact 값으로 계속 보유합니다: `detectionRangeWorld=15`, `exploreArrivalDistanceWorld=0.25`, `retreatHPRatio=0.2`, `retreatDistanceWorld=6`, `projectileLookAheadWorld=8`, `dodgeMarginWorld=0.35`. SL-116 자체에서는 Client config v3와 REST/OpenAPI/AsyncAPI field/event shape is unchanged였고 AsyncAPI info version `0.7.0`을 유지했습니다. 현재 SL-120은 Client config v3와 REST/OpenAPI를 유지하며 AsyncAPI info version을 `0.10.0`으로 올립니다.


SL-123 대시 상태는 `timedDashState`가 소유해요. 입력 없는 tick에도 진행하며 승인 tick과 마지막 tick을 포함한 9개 구간에서 일반 이동·공격을 차단해요. 공개 `IsDashing`과 `AttackReadyTick=A+9`는 마지막 구간/충돌/사망 snapshot에 `false/0`이 돼요. Projectile pre-phase 사망, melee post-dash 사망, `EliminatePlayers`와 초기 player 정규화도 stale 대시 상태를 남기지 않아요. Colt/Lily의 승인 및 projectile origin은 기존 일반 이동 후 phase를 유지해요.
