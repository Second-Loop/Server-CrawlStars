...
이 레포지토리는 E1 서버 권위 core loop skeleton을 main에 반영했고, 지금은 E2 client-server integration에 필요한 development surface를 좁게 확장하는 단계입니다. 서버 architecture는 아직 의도적으로 작게 유지합니다.
...
다음 architecture 작업은 SL-12/SL-30처럼 남아 있는 E2 논의를 작은 ticket으로 잘라서 진행합니다.

- match ready/loading/countdown state transition
- start 전 cancel과 ready timeout
- shared constants/config source of truth
- client/server validation 및 test strategy
- observability basics

Gameplay server architecture를 production queue, persistence, scheduler/runner framework로 과도하게 일반화하지 않습니다.
...
- Match complete event, client loading/ready ACK, countdown start, start 전 cancel 처리는 아직 구현하지 않았으며 `SL-58` 범위입니다.
