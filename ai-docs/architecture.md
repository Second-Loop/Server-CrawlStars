...
- 최종 이동 후보가 다른 live player와 다음 tick에 충돌하면 bot은 현재 진행 방향 기준 `+90°`, `-90°` 순서로 map/player-safe 후보를 고릅니다. 같은 previous snapshot을 보는 정면 충돌 bot은 서로 반대 world 방향으로 비켜나며, 후보가 모두 막히면 zero movement입니다. Simulation의 공통 player collision 판정은 변경하지 않습니다.
