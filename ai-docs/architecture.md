...
Skill-ready와 non-zero direction이면 normal attack보다 먼저 승인합니다. Server config v6의 typed `SkillConfig`가 `reload_dash`/`burst_projectile`/`teleport_projectile` payload를 소유하고 승인 경계가 이를 character executor에 전달합니다. Shelly executor는 charge/recharge를 `max/0`으로 만든 뒤 일반 이동이 끝난 위치에서 `5.4 tile` swept dash intent를 만들고, 모든 same-tick dash를 연속 시간 batch로 정산합니다. Wall/Water/boundary/live player는 막고 dead player는 통과하며 partial/blocked dash도 reload와 cooldown을 유지합니다. Colt executor는 기존 tick scheduler의 active normal burst를 skill burst로 교체하고 승인 방향을 고정해 `S+[0,2,4,6,7,9,11,13,14,16,18,20]`에 `colt_skill` 12발을 생성합니다. 각 발은 emission tick의 post-movement 위치에서 시작하고 마지막 발 tick까지 일반 공격의 charge 소비와 queue를 막습니다. Cooldown 또는 zero direction이면 기존 normal attack 판정으로 fall through합니다. Public AsyncAPI는 `0.8.0`이며 gameplay `PlayerData.PressedSkill`/`SkillReadyTick`/`AttackCharges`/`NextAttackChargeTick`, 기존 `Pos`와 `Projectiles[].Type`으로 결과를 공개합니다. REST OpenAPI와 starting/started control의 `Players: null`, `Projectiles: null`은 유지하고 Lily effect와 bot skill use는 후속 티켓 범위입니다.
