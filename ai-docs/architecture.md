...
Skill-ready와 non-zero direction이면 normal attack보다 먼저 승인하고 attack charge를 보존합니다. Cooldown 또는 zero direction이면 기존 normal attack 판정으로 fall through합니다. Server config v6의 typed `SkillConfig`가 `reload_dash`/`burst_projectile`/`teleport_projectile` payload를 소유하고 승인 경계가 이를 effect dispatch에 전달합니다. Public AsyncAPI는 `0.8.0`이며 gameplay `PlayerData.PressedSkill`/`SkillReadyTick`/`AttackCharges`/`NextAttackChargeTick`을 required로 두지만, REST OpenAPI와 starting/started control의 `Players: null`, `Projectiles: null`은 유지합니다. 실제 skill effect와 bot skill use는 후속 티켓 범위입니다.
