    ...
    content: `
      ...
      <section class="panel">
        ...
        <div class="grid">
          ...
          <article>
            ...
            <p>Control snapshot의 <code>Players: null</code>과 <code>Projectiles: null</code>을 유지하고 gameplay entity를 넣지 않습니다. 현재 Shelly <code>reload_dash</code>와 Colt <code>burst_projectile</code>을 실행하고 Lily effect와 bot skill use는 아직 실행하지 않습니다. Client config v3/server config v6 경계를 유지합니다.</p>
          </article>
          <article>
      ...
      <section class="panel">
        ...
        <div class="grid">
          <article>
            <h3>Session token</h3>
            <p>Token은 일회용이 아니며 room/player session이 남아 있을 때 재사용합니다. Unmatched disconnect는 room-owned 10초 fill deadline과 credential을 유지합니다. Matchmaking match가 완성되면 strict 30초 matched human attach deadline을 적용하고, 만료되면 pre-start room 전체를 취소합니다. Client는 fresh room/player/session identity를 받기 위해 새 join을 해야 하며, Idempotency-Key replay는 제공하지 않습니다. matched/loading/starting disconnect는 pre-start cancel로 room을 삭제하고, failed upgrade는 같은 경로로 재시도할 수 있습니다.</p>
          </article>
          <article>
            <h3>실패 순서</h3>
