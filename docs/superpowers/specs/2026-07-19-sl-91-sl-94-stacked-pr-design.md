...
WebSocket upgrade나 attach가 완료되기 전 실패는 현재 재시도 동작을 유지하고 timer도 계속 진행합니다. 실제 attach가 끝난 human이 start 전에 disconnect하면 기존 pre-start match cancel로 room을 삭제하고 timer도 중단합니다. Disconnect replacement bot이나 별도 reconnect grace는 추가하지 않습니다.
