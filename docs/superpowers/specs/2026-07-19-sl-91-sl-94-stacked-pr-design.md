...
- 두 PR 모두 Linear issue 하나의 범위만 담고, 각각 독립적으로 검토할 수 있게 유지합니다.
...
- Bot fill이 먼저면 정원을 bot으로 채워 room을 닫고, 늦은 human은 새 waiting room으로 보냅니다.
- 요청 시작 timestamp를 따로 저장하거나 보정하지 않습니다.

### SL-94의 비단조 ClientTick

- 양수 `ClientTick`이 이미 처리한 tick 이하이면 입력 전체를 조용히 무시합니다.
- 같은 player에게 더 큰 양수 pending tick이 있으면 그 이하의 새 입력도 조용히 무시합니다.
...
첫 human이 REST matchmaking join으로 새 room을 만든 시점에 room-owned one-shot ticker를 시작합니다. 후속 join이나 WebSocket attach는 deadline을 다시 설정하지 않습니다.

기존 clock/ticker 추상화를 사용해 fake clock으로 경계를 검증합니다. 중앙 scheduler나 실제 `time.AfterFunc`는 추가하지 않습니다. Worker는 Store의 worker wait group에 등록해 Shutdown이 종료를 기다릴 수 있게 합니다.
...
Waiting room의 human WebSocket이 잠시 끊겨도 현재 credential/reconnect 동작을 유지하고 timer는 계속 진행합니다. Disconnect replacement bot이나 별도 reconnect grace는 추가하지 않습니다.
...
Bot ID 생성은 append 전에 필요한 수만큼 모두 성공해야 합니다. 생성 실패 시 일부만 추가하지 않고 오류를 기록한 뒤 waiting 상태를 유지합니다. 별도 retry worker는 만들지 않으며 이후 human join이나 기존 TTL cleanup이 room을 처리합니다.
...
- Bot fill 이후 들어오는 human join은 해당 room을 건너뛰고 새 waiting room을 선택하거나 만듭니다.
...
- Barrier test로 human-first와 timer-first 순서를 각각 고정해 중복, overflow, late join routing을 확인합니다.
- Duel, Solo, Team의 모든 대표 human 수에서 slot/team 배치를 확인합니다.
...
Snapshot의 `PlayerData`에는 required `LastProcessedClientTick int64`를 추가합니다. 최초 값과 bot 값은 `0`입니다. `Snapshot.Tick`의 server simulation tick 의미와 starting snapshot의 `Players: null` 동작은 바꾸지 않습니다.
...
2. 양수 tick이 마지막으로 적용된 ACK 이하이면 stale input으로 조용히 버립니다.
3. 양수 pending input이 이미 있으면 그 tick 이하의 새 양수 입력을 조용히 버립니다.
4. 더 큰 양수 tick은 기존 pending input을 덮어씁니다.
5. `0`인 legacy input은 기존 last-write-wins 동작을 유지합니다.
...
- 존재하고 살아 있는 player에게 유효한 input을 실제 적용할 때만 양수 `ClientTick`으로 ACK를 증가시킵니다.
...
- `Snapshot.Tick`과 starting `Players: null` 회귀가 없는지 확인합니다.
- Targeted 반복·race test와 전체 `make ci`를 실행합니다.
