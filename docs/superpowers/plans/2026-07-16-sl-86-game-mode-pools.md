...
**Tech Stack:** Go 1.25, `net/http`, embedded JSON server config, OpenAPI 3.1, AsyncAPI 3.0, repository `make ci` validation.
...
`readyEventDeliveries`, `readyEventPlayers`, `simulationPlayers`, `startRoomLocked`, Ready/client capacity checks, gameplay tick interval, `room.gameEndResults`가 모두 caller가 lock한 room의 `gameConfig`를 사용하게 바꿔요. Store `gameConfig`는 새 room의 default/catalog source로만 남겨요. State config resolve는 같은 selected ID의 입력 payload를 신뢰하지 않고 catalog의 canonical entry로 복원해요.
...
```bash
git add internal/rooms/store.go internal/rooms/messages.go internal/rooms/websocket.go internal/rooms/websocket_test.go internal/rooms/game_end.go internal/rooms/game_end_test.go internal/simulation/game_config.go internal/simulation/game_config_test.go
...
- Modify: `api/asyncapi.yaml` (기존 Team field의 allowed values와 mode-aware 설명만 확장)
...
- Modify: `docs/superpowers/plans/2026-07-16-sl-86-game-mode-pools.md` (실제 version/file scope drift 정정)
- Generated and gitignored: `internal/docs/api/openapi.yaml`, `internal/docs/api/asyncapi.yaml`
...
`docs-ui/scripts/validate.mjs`에는 optional request body, request schema ref, 400 named example별 exact error code, response required fields, Player team enum을 schema block 단위로 검증하는 assertion을 추가해요. `internal/docs/docs_test.go`에는 다음 embedded OpenAPI marker를 검증해요.
...
Expected: source validator는 optional request body/schema 부재로, embedded docs test는 generated OpenAPI의 marker 부재로 실패해요.
...
`Room`과 기존 response schema `MatchmakingJoin`의 required field에 `gameMode`를 넣고, 400 examples로 `invalid_game_mode`와 `invalid_request`를 추가해요. OpenAPI `Player.team`과 기존 AsyncAPI `ReadyPlayer.Team`/`PlayerData.Team`은 red/blue와 solo-1부터 solo-6을 모두 허용해요. AsyncAPI는 field shape를 바꾸지 않고 fixed 2-client 설명만 selected mode required player 기준으로 고쳐요. `api-reference.md`는 no-body/empty compatibility와 예제 request를, `protocol.md`는 same-mode pool과 room-local config invariant를, `architecture.md`는 catalog와 room ownership을, `project-map.md`는 현재 활성 계약을 기록해요. `decisions.md`에는 mode catalog를 Store global active mode가 아니라 room-local selected config로 고정하는 ADR을 추가해요.
...
```bash
git add api/openapi.yaml api/asyncapi.yaml ai-docs/api-reference.md ai-docs/protocol.md ai-docs/architecture.md ai-docs/project-map.md ai-docs/decisions.md docs-ui/scripts/validate.mjs internal/docs/docs_test.go docs/superpowers/plans/2026-07-16-sl-86-game-mode-pools.md
