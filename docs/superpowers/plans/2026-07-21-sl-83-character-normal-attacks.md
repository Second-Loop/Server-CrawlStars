...
Add table tests that attack until exhaustion and assert accepted counts `Shelly=3`, `Colt=1` while its burst is active, and `Lily=2`; test raw charge capacity through a package-private helper to separate Colt non-overlap from capacity:
