...
기존 player-level `maxAttackCharges`와 `attackRechargeTicks`는 v3에서 제거해 source of truth를 하나로 만들어요. Projectile type은 `id/radius/speed`만 소유하고 damage는 normal attack이 소유해요. Public `ProjectileData.Damage`와 `Type`은 유지하며 생성 시 attack damage와 referenced projectile type을 복사해요.
...
Colt의 charge recharge는 active burst와 독립적으로 계속 진행해요. 마지막 emission tick에 charge가 회복되더라도 그 tick의 재공격은 active burst 때문에 거절하고 다음 tick부터 사용할 수 있어요.

Go map 순회 순서는 projectile ID나 snapshot 순서에 사용하지 않아요. Same-tick emission은 owner ID와 ordinal로 정렬하고 melee damage는 canonical player index slice에 누적해 input slice 순서가 바뀌어도 결과가 같아야 해요.
...
- config v3 exact catalog, invalid 조합, static fallback, client/server version 분리
...
- range endpoint hit, wall priority, expiry, friendly-fire/mode matrix
